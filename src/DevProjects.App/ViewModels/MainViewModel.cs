...
public sealed partial class MainViewModel : ObservableObject
{
        ...
        IClaudeSessionDetector? sessionDetector = null)
    {
        ...
        _watcherDebounce = new DispatcherTimer(DispatcherPriority.Background, dispatcher)
        {
            Interval = TimeSpan.FromMilliseconds(1500),
        };
        _watcherDebounce.Tick += (_, _) => { _watcherDebounce.Stop(); Rescan(); };

        // Live-session indicator refresh: cheap mtime probes every 30 s.
        _runningRefreshTimer = new DispatcherTimer(DispatcherPriority.Background, dispatcher)
        {
        ...
        _flagsSaveDebounce = new DispatcherTimer(DispatcherPriority.Background, dispatcher)
        {
            Interval = TimeSpan.FromMilliseconds(500),
        };
        _flagsSaveDebounce.Tick += (_, _) => { _flagsSaveDebounce.Stop(); FlushPendingFlagsSave(); };
    ...
    [RelayCommand]
    private void LaunchNew(ProjectItemViewModel? project) => Launch(project, continueSession: false);

    [RelayCommand]
    private void LaunchContinue(ProjectItemViewModel? project) => Launch(project, continueSession: true);

    private void Launch(ProjectItemViewModel? project, bool continueSession)
    {
        if (project is null) return;
        FlushPendingFlagsSave();
        if (!LaunchCommandBuilder.AreFlagsSafe(project.Flags))
        {
            MessageBox.Show(LaunchCommandBuilder.UnsafeFlagMessage, "Dev-Projects",
        ...
        {
            MessageBox.Show($"Launch failed: {ex.Message}", "Dev-Projects",
    ...
    public void LaunchInFolder(string folderPath)
    {
    ...
    [RelayCommand]
    private void MoveSelectedToRoot(string? targetRoot)
    {
        ...
        var confirm = MessageBox.Show(
            ...
            "Move project", MessageBoxButton.YesNo, MessageBoxImage.Question);
        ...
        {
            MessageBox.Show($"Could not move the project: {ex.Message}\n\n" +
                "If a Claude session or another program has files open in this folder, close it and try again.",
                "Move project", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }
    ...
    private void FinishRelocation(string oldPath, string newPath)
    {
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            MessageBox.Show(
                ...
                + ex.Message, "Dev-Projects", MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        Rescan();
    ...
    private void RebuildWatchers()
    {
        ...
        foreach (var root in _config.Roots ?? [])
        {
            ...
            try
            {
                ...
                FileSystemEventHandler onChange = (_, _) => _dispatcher.BeginInvoke(() =>
                {
                ...
                watcher.Renamed += (_, _) => _dispatcher.BeginInvoke(() =>
                {
                    _watcherDebounce.Stop();
                    _watcherDebounce.Start();
                });
